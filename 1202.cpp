...
해당 프로그램 진행과정을 요약하자면
첫번째로 가장 무게를 적게 버티는 가방에 들어가는 보석들을 우선순위 큐에 넣는다.
두번째로 우선순위 큐에 2개이상의 보석에 들어가 있다면, 앞서 언급했듯이 가장 무게를 적게 버티는 가방을 처음 사용했고
그 이후 가방은 그 가방보다 높은 무게를 버틸수 있으므로 우선순위 큐에 있는 모든 보석을 넣을수 있다. 
따라서 우선순위 큐에 있는 보석들을 가방에 넣는다.
마지막으로 해당과정을 반복하여 보석의 총합을 구한다. 
*/
